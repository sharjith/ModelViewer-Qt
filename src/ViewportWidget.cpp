	...
	_assimpModelLoader(nullptr)
{
	...
	// _measurementController deliberately does NOT take _axisTextRenderer as
	// a dependency (see MeasurementController's constructor doc comment) -
	// that's still nullptr at this point anyway (only set later, in
	// initializeGL()) - so it's safe to construct here, alongside
	// _transformGizmo, rather than needing to wait.
	_measurementController = new MeasurementController(_sceneRuntime, _viewer, _renderCtrl, this);
	_gpuResourceRegistry.add(_measurementController, GpuResourcePhase::Decorations);
	connect(_measurementController, &MeasurementController::measurementToolChanged,
		this, &ViewportWidget::measurementToolChanged);
	connect(_measurementController, &MeasurementController::measurementProgressChanged,
		this, &ViewportWidget::measurementProgressChanged);
	connect(_measurementController, &MeasurementController::measurementSelectionChanged,
		this, &ViewportWidget::measurementSelectionChanged);
...
// ---------------------------------------------------------------------------
// Measurement tool - thin forwards to _measurementController, which owns the
// whole toolset (see MeasurementController.h/.cpp). setMeasurementTool()/
// setSelectedMeasurementIds() need real bodies (repaint + signal relay after
// calling the controller); the other 5 public methods are trivial one-liners
// already inlined in ViewportWidget.h.
// ---------------------------------------------------------------------------
...
void ViewportWidget::setSelectedMeasurementIds(const QSet<QUuid>& ids)
{
	if (!_measurementController)
		return;
	_measurementController->setSelectedMeasurementIds(ids);
}
...
void ViewportWidget::mousePressEvent(QMouseEvent* e)
{
	...
	if (e->button() & Qt::LeftButton)
	{
		...
		if (_measurementController->measurementTool() != MeasurementTool::None
			...
			&& !_viewCtrl.viewPanning() && !_viewCtrl.viewZooming())
		{
			_measurementController->setMeasurementClickCandidate(true);
			_measurementController->setMeasurementClickPressPos(clickPoint);
			return;
		}
		...
		else if (_measurementController->measurementTool() == MeasurementTool::None
			&& !(e->modifiers() & Qt::ControlModifier) && !(e->modifiers() & Qt::ShiftModifier)
			&& !_viewCtrl.windowZoomActive() && !_viewCtrl.viewRotating()
			&& !_viewCtrl.viewPanning() && !_viewCtrl.viewZooming())
		{
			...
			if (hitDimension.kind != MeasurementController::DimensionDragKind::None)
			{
				_measurementController->setDimensionDragCandidate(true);
				_measurementController->setDimensionDragCandidateId(hitDimension.measurementId);
				_measurementController->setDimensionDragCandidateKind(hitDimension.kind);
				_measurementController->setDimensionDragStartPixel(clickPoint);
				return;
			}

			const QUuid hitMeasurement = _measurementController->hitTestMeasurement(clickPoint, _primaryCamera, QSize(width(), height()), 8);
...
void ViewportWidget::mouseReleaseEvent(QMouseEvent* e)
{
	...
	if ((e->button() & Qt::LeftButton) && _measurementController->dimensionDragCandidate())
	{
		if (_measurementController->dimensionDragActive())
		{
			_measurementController->finishDimensionLineDrag(this);
		}
		else
		{
			...
			setSelectedMeasurementIds({ _measurementController->dimensionDragCandidateId() });
			if (_selectionManager)
				_selectionManager->setSelectedIds({});
			_measurementController->setDimensionDragCandidate(false);
			_measurementController->setDimensionDragCandidateId(QUuid());
			_measurementController->setDimensionDragCandidateKind(MeasurementController::DimensionDragKind::None);
		}
		update();
		return;
	}

	if ((e->button() & Qt::LeftButton) && _measurementController->measurementClickCandidate())
	{
		_measurementController->setMeasurementClickCandidate(false);
...
void ViewportWidget::mouseMoveEvent(QMouseEvent* e)
{
	...
	if (_measurementController->dimensionDragCandidate() && (e->buttons() & Qt::LeftButton))
	{
		if (!_measurementController->dimensionDragActive())
		{
			const QPoint moved = e->pos() - _measurementController->dimensionDragStartPixel();
			if (moved.manhattanLength() >= kCameraDragThresholdPx)
				_measurementController->beginDimensionLineDrag(_measurementController->dimensionDragCandidateId(),
					_measurementController->dimensionDragCandidateKind(), _primaryCamera);
		}
		if (_measurementController->dimensionDragActive())
		{
	...
	if (e->buttons() == Qt::NoButton && _measurementController->measurementTool() != MeasurementTool::None && _selectionManager)
	{
		_measurementController->updateHoverAnchor(e->pos(), _selectionManager);
	}
	...
	else if (e->buttons() == Qt::NoButton && _measurementController->measurementTool() == MeasurementTool::None && !gizmoHovered
