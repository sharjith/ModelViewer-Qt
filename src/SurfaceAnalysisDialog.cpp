	...
	, _modelViewer(modelViewer)
{
	...
	// Deviation page - placeholder only.
	{
		auto* page = new QWidget();
		auto* pageLayout = new QVBoxLayout(page);
		auto* note = new QLabel(tr("Deviation (rev-to-rev / scan-vs-CAD comparison) is not yet available."), page);
		note->setWordWrap(true);
		pageLayout->addWidget(note);
		pageLayout->addStretch(1);
		_stack->addWidget(page);
...
{
	_stack->setCurrentIndex(static_cast<int>(currentMode()));
	// The legend only means something for a fixed-scale colormapped result
	// (Draft Angle so far) - hide it for Curvature (Zebra Stripe has no
	// scale) and Deviation (not yet implemented) so a stale legend from a
	// previous Draft Angle run doesn't linger looking like it applies here.
	_legendLabel->setVisible(false);
}
